    ...
    while keep_training:
        ...
        for batch_num, ((source_scales, source_label), target_scales) in enumerate(zip(opt.source_loaders[opt.curr_scale], opt.target_loaders[opt.curr_scale])):
            ...
            if True:
                ...
                force_save_dir = "/home/work/inAI_plusStyle/ProCST/output_images"
                if not os.path.exists(force_save_dir):
                    os.makedirs(force_save_dir)

                try:
                    vutils.save_image(sit, os.path.join(force_save_dir, f"step_{steps}.png"))
                    print(f">>> [SUCCESS] Image saved: step_{steps}.png")
